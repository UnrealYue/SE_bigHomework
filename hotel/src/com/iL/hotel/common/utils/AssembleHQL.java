...
public class AssembleHQL {
	...
	*/
	...
	public static Query createQuery(Session session, String hql, Map<String, ?> values,Sorter sorter)
	{
		Assert.hasText(hql, "hql不能为空");
		Query query = session.createQuery(hql + sorter.getOrderHqlString());
		if(DetectionUtils.isEmpty(values))
			return query;
		Iterator<String> iter = values.keySet().iterator();
		while(iter.hasNext()){
	...
	public static Query createQuery(Session session, String hql, Map<String, ?> values,Sorter sorter,Grouper grouper)
	{
		Assert.hasText(hql, "hql不能为空");
		Query query = session.createQuery(hql + sorter.getOrderHqlString() + grouper.getGroupHql());
		if(DetectionUtils.isEmpty(values))
			return query;
		Iterator<String> iter = values.keySet().iterator();
		while(iter.hasNext()){
